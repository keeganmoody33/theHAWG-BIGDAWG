...
The Context OS is a two-layer knowledge architecture where AI compounds intelligence over time. Layer 1 (knowledge_base/) holds atomic, reusable concepts linked via `[[wiki-links]]`. Layer 2 (00_foundation/) holds operational documents that compose from the graph — they reference atomic concepts, they don't redefine them.
...
- **Layer 1: Knowledge Graph** (`knowledge_base/`) — atomic concepts with frontmatter metadata
  ...
  - Linked via `[[wiki-links]]` — the graph has structure, not just files
